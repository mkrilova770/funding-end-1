...
В репозитории добавлен **`railway.toml`**: перед стартом деплоя выполняется **`npm run db:push`** (создаётся таблица `TelegramDigestConfig` и остальная схема). Сделайте **новый деплой** (Redeploy / push в `main`) — после успешного деплоя окно Telegram должно открываться.

Если всё равно ошибка — выполните `db:push` вручную (варианты ниже).

### Где вручную на Railway (если нет Shell в браузере)

В веб-интерфейсе Railway **часто нет** отдельной кнопки «консоль» на всех тарифах. Проще всё сделать **с компьютера через CLI**:

1. Установи CLI: `npm i -g @railway/cli`
2. Войди: `railway login`
3. В папке проекта: `railway link` — выбери **проект** и **сервис**, у которого в Variables тот же **`DATABASE_URL`**, что у сайта.
4. Выполни (подставятся переменные с облака, команда идёт у тебя локально, но к **облачной** БД):

```bash
railway run npm run db:push
```

5. Обнови сайт и снова открой **Telegram** в шапке.

**Альтернатива:** в контейнер деплоя (если CLI уже настроен):

```bash
railway ssh --service <имя-сервиса-сайта>
