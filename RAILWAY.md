...
## Окно Telegram / схема БД без ручных команд

Команды **`npm start`** (сайт) и **`npm run worker`** (воркер) сами выполняют **`prisma db push`** перед запуском. После деплоя на Railway таблицы (включая `TelegramDigestConfig`) подтягиваются **автоматически**, пока у сервиса задан корректный **`DATABASE_URL`**. Достаточно **Redeploy** или нового коммита — ничего вручную в консоли Railway не нужно.

Если ошибка остаётся — проверьте `DATABASE_URL` и логи деплоя; при необходимости вручную: `railway run npm run db:push` с CLI.
...
Схема Prisma применяется **при старте** веба и воркера автоматически (`prisma db push` в `npm start` / `npm run worker`).
